...
from extensions.chamfer_dist import ChamferDistanceL1, ChamferDistanceL1_PM, ChamferDistanceL2
from extensions import emd
from extensions.emd import emd_module
...
class MultiLayerVisionTransformer(nn.Module):
    """
    多层图像 Encoder (ViT), patch embed + L 层 Transformer Block
    ...
    def __init__(self, img_size=256, patch_size=16, in_chans=3,
        ...
        for i in range(depth):
            block_list.append(nn.TransformerEncoderLayer(
                d_model=embed_dim,
                nhead=num_heads,
    ...
    def forward(self, x):
        """
        x: [B, 256, 256, 3]
        return: [B, (1 + num_patches), embed_dim]  # patch tokens + cls_token
        """
        B, H, W, C = x.shape
        x = self.patch_embed(x)    # => [B, embed_dim, H', W']
        # flatten => [B, embed_dim, #patch], => transpose => [B, #patch, embed_dim]
        x = x.flatten(2).transpose(1,2)
                                              
        # prepend cls_token
        cls_token = self.cls_token.expand(B, -1, -1)   # [B, 1, embed_dim]
        x = torch.cat((cls_token, x), dim=1)  # => [B, 1+#patch, embed_dim]
...
class ViewFeatureFusionModule(nn.Module):
    def __init__(self, embed_dim):
        super(ViewFeatureFusionModule, self).__init__()
        self.mlp = nn.Sequential(
            nn.Linear(embed_dim, embed_dim),
            nn.ReLU(inplace=True),
            nn.Linear(embed_dim, embed_dim)
        )
        self.attn_layer = nn.MultiheadAttention(embed_dim, num_heads=4, batch_first=True)

    def forward(self, point_features, image_features):
        """
        point_features: [B, N, embed_dim] 点云特征
        image_features: [B, num_patches, embed_dim] 图像特征
        pos: [B, N, 3] 点云的坐标
        query: 用于自注意力的查询 (如果需要的话)
        """
        if image_features.dim()<2:
            image_features = image_features.unsqueeze(0)
        view_feature = self.mlp(image_features)  # MLP生成视图级别特征
        # 使用点云特征与图像视图特征的自注意力融合
        attn_output, _ = self.attn_layer(point_features, view_feature, view_feature)

        # 结合位置编码对特征进行对齐，增强视角感知能力
        fused_features = attn_output + point_features  # 加上原始点云特征
        
        return fused_features


class DinoViTEncoder(nn.Module):
    def __init__(self, pretrained_path: str, device='cuda'):
        super().__init__()
        self.device = device

        # 加载本地预训练模型
        self.base = dinov2_vitb14(weights={'LVD142M': pretrained_path}).to(device)
        self.embed_dim = self.base.embed_dim  # ViT-B/14 的 embed_dim 为 768

        # 使用 ModelWithIntermediateLayers 获取中间层输出
        autocast_ctx = partial(torch.cuda.amp.autocast, enabled=True, dtype=torch.float16)
        self.feature_model = ModelWithIntermediateLayers(self.base, n_last_blocks=1, autocast_ctx=autocast_ctx).to(device)

        # 冻结所有参数
        for param in self.feature_model.parameters():
            param.requires_grad = False

        # 解冻最后两个 Transformer blocks
        for name, param in self.base.named_parameters():
            if 'blocks.10' in name or 'blocks.11' in name:
                param.requires_grad = True

        # 定义输出层，将特征维度映射到 1024
        self.projection = nn.Linear(self.embed_dim, 1024)

    def forward(self, x):
        # x: [batch, 3, 256, 256]
        with torch.cuda.amp.autocast(enabled=True, dtype=torch.float16):
            features = self.feature_model(x)
            feature_list = features[0]        # 拿到内部的多个层输出
            out = self.projection(feature_list[-1])   # [B, 1024]
...
class PCTransformer(nn.Module):
    def __init__(self, config):
        ...
        # 加载dinov2模型  
        self.img_feature_extractor = DinoViTEncoder("./models/dinov2_vitb14_pretrain.pth")
        self.view_feature_fusion = ViewFeatureFusionModule(embed_dim=1024)
    ...
    def forward(self, xyz, image):
        ...
        x = self.input_proj(f) # b n 384
        x = self.encoder(x + pe, coor) # b n 384
        pc_feat = self.increase_dim(x) # B 1024 N 
        pc_feat = torch.max(pc_feat, dim=1)[0] # B 1024

        # ---- 图像预处理与编码 ----
        depth_feat = self.img_feature_extractor(image)
  
        # 交叉注意力融合特征
        fused_features = self.view_feature_fusion(pc_feat, depth_feat)
        
        # ----- 4) 生成 coarse点云-----   
        coarse = self.coarse_pred(fused_features).reshape(bs, -1, 3) # B points 3
        ...
        coarse = torch.gather(coarse, 1, idx[:,:self.num_query].expand(-1, -1, coarse.size(-1))) # B 128 3
        # produce query
        q = self.mlp_query(
        torch.cat([
            pc_feat.unsqueeze(1).expand(-1, coarse.size(1), -1),
            coarse], dim = -1)) # b n c
...
class UniDCF(nn.Module):
    ...
    def build_loss_func(self):
        ...
        self.dist = emd_module.emdModule()

    def get_loss(self, ret, gt):
        pred_coarse, pred_fine = ret
        assert pred_fine.size(1) == gt.size(1)
        # recon loss
        loss1_coarse = self.loss_func(pred_coarse, gt)
        loss1_fine = self.loss_func(pred_fine, gt)

        return loss1_coarse, loss1_fine
        
    def forward(self, xyz, image):
        q, coarse_point_cloud = self.base_model(xyz, image) # B M C and B M 3
